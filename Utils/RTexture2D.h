...
class RTexture2D : public RTexture
{
    ...
    public:
        RTexture2D(string path, unsigned char* data, TextureFormat internalFormat, glm::uvec2 size, bool fromFile, bool useCompression = false);
        RTexture2D(string path, float* data, TextureFormat internalFormat, glm::uvec2 size, bool fromFile, bool useCompression = false);
        RTexture2D(TextureFormat internalFormat, glm::uvec2 size, bool isMultisample = false, int samplesCount = 0);
		RTexture2D(string path, GLuint textureId, glm::uvec2 size, TextureFilterMode minFilter, TextureFilterMode magFilter, bool fromFile); // for dds texture
        virtual ~RTexture2D();
