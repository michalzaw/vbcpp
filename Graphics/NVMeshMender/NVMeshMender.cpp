...
public:
	virtual bool CanSmooth(MeshMender::Triangle* t1, MeshMender::Triangle* t2, const float& minCreaseAngle)
	{
		...
		tmp1 = glm::normalize(tmp1.GLMvec());//D3DXVec3Normalize( &tmp1, &tmp1);
		tmp2 = glm::normalize(tmp2.GLMvec());//D3DXVec3Normalize( &tmp2, &tmp2);

		if(glm::dot(tmp1.GLMvec(), tmp2.GLMvec()) >= minCreaseAngle )
		{
			return true;
		}
								...
								D3DXVECTOR3 workingPosition)
{
		...
		for(i=0; i<neighborGroups.size(); ++i)
		{
			...
			gtang = glm::normalize(gtang.GLMvec());//D3DXVec3Normalize( &gtang, &gtang );
								  ...
								  const NormalCalcOption computeNormals)
{
	...
	if(computeNormals == CALCULATE_NORMALS)
	{
		...
		t.normal = glm::cross(edge0.GLMvec(), edge1.GLMvec());//D3DXVec3Cross( &t.normal, &edge0, &edge1);

		if( WeightNormalsByArea < 1.0f )
		{

			D3DXVECTOR3 normalizedNorm;
			normalizedNorm= glm::normalize(t.normal.GLMvec());//D3DXVec3Normalize(&normalizedNorm,&t.normal);
						...
						std::vector< Vertex >&   theVerts )
{
	...
	//if (!(glm::length(theVerts[i].normal.GLMvec()) > 0.00001f)){
        //double x = glm::length(theVerts[i].normal.GLMvec());
        //std::cout << x << std::endl;
	//}
        //assert( D3DXVec3Length(&(theVerts[i].normal)) > 0.00001f &&
		/*assert( glm::length(theVerts[i].normal.GLMvec()) > 0.00001f &&
		...
		D3DXVECTOR3 newT = tmpTan -  (glm::dot(tmpNorm.GLMvec(), tmpTan.GLMvec())/*D3DXVec3Dot(&tmpNorm , &tmpTan)*/  * tmpNorm );
		D3DXVECTOR3 newB = tmpBin - (glm::dot(tmpNorm.GLMvec(), tmpBin.GLMvec())/*D3DXVec3Dot(&tmpNorm , &tmpBin)*/ * tmpNorm)
							- (glm::dot(newT.GLMvec(), tmpBin.GLMvec())/*D3DXVec3Dot(&newT,&tmpBin)*/*newT);

		theVerts[i].tangent = glm::normalize(newT.GLMvec());//D3DXVec3Normalize(&(theVerts[i].tangent), &newT);
		theVerts[i].binormal = glm::normalize(newB.GLMvec());//D3DXVec3Normalize(&(theVerts[i].binormal), &newB);

		//this is where we can do a final check for zero length vectors
		//and set them to something appropriate
		float lenTan = glm::length(theVerts[i].tangent.GLMvec());//D3DXVec3Length(&(theVerts[i].tangent));
		float lenBin = glm::length(theVerts[i].binormal.GLMvec());//D3DXVec3Length(&(theVerts[i].binormal));
		...
		if( (lenTan <= 0.001f) || (lenBin <= 0.001f)  ) //should be approx 1.0f
		{
			...
			if(lenTan > 0.5f)
			{
				...
				theVerts[i].binormal = glm::cross(theVerts[i].normal.GLMvec(), theVerts[i].tangent.GLMvec());//D3DXVec3Cross(&(theVerts[i].binormal), &(theVerts[i].normal), &(theVerts[i].tangent) );
			...
			else if(lenBin > 0.5)
			{
				...
				//the tangent
				theVerts[i].tangent = glm::cross(theVerts[i].binormal.GLMvec(), theVerts[i].normal.GLMvec());//D3DXVec3Cross(&(theVerts[i].tangent), &(theVerts[i].binormal), &(theVerts[i].normal) );
			}
			else
			{
				...
				if( glm::dot(xAxis.GLMvec(), theVerts[i].normal.GLMvec() )  <  glm::dot(yAxis.GLMvec(), theVerts[i].normal.GLMvec() ) )
				{
				...
				theVerts[i].tangent = glm::cross(theVerts[i].normal.GLMvec(), startAxis.GLMvec() );
				theVerts[i].binormal = glm::cross(theVerts[i].normal.GLMvec(), theVerts[i].tangent.GLMvec() );
		...
		else
		{
			...
			if( glm::dot(theVerts[i].binormal.GLMvec(), theVerts[i].tangent.GLMvec() )  > 0.999f )
			{
